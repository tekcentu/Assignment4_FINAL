"""Tool-mode controllers — translate canvas events into model commands.

Each tool implements the same minimal interface (``on_click(hit, button)``,
``on_motion(hit)``, ``description``). The MainApplication switches the active
tool when the user clicks a toolbar button or presses a keyboard shortcut.
"""
...
class _Host(Protocol):
    """Protocol for what a tool can ask the application to do."""

    def execute(self, cmd) -> None: ...
    def model(self): ...
    def show_node_menu(self, node_id: int, x: int, y: int) -> None: ...
    def show_element_menu(self, elem_id: int, x: int, y: int) -> None: ...
    ...
    def on_click(self, hit: HitResult, button: str) -> None:
        if button == "right":
            if hit.node_id is not None:
                self.host.show_node_menu(hit.node_id, 0, 0)
            elif hit.element_id is not None:
                self.host.show_element_menu(hit.element_id, 0, 0)
...
class _PairTool(Tool):
    """Two-click tool: pick a pair of nodes."""

    def __init__(self, host: _Host, kind: str) -> None:
        super().__init__(host)
        self.kind = kind  # "frame" or "truss"
    ...
    def on_click(self, hit: HitResult, button: str) -> None:
        ...
        if hit.node_id is None:
            self.host.set_status(f"Click an existing node to {'start' if self._first is None else 'finish'} the element.")
            return
        if self._first is None:
...
class SupportTool(Tool):
    ...
    def on_click(self, hit: HitResult, button: str) -> None:
        if button != "left" or hit.node_id is None:
            return
        self.host.show_node_menu(hit.node_id, 0, 0, action="support")
...
class NodalLoadTool(Tool):
    ...
    def on_click(self, hit: HitResult, button: str) -> None:
        if button != "left" or hit.node_id is None:
            return
        self.host.show_node_menu(hit.node_id, 0, 0, action="nodal_load")
...
class MemberLoadTool(Tool):
    ...
    def on_click(self, hit: HitResult, button: str) -> None:
        if button != "left" or hit.element_id is None:
            return
        self.host.show_element_menu(hit.element_id, 0, 0, action="member_load")
